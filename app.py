...
# =============================================================================
# CONFIGURATION - ADDED NEW CONFIGURATIONS
# =============================================================================
class Config:
...
# =============================================================================
# DATABASE MODELS - ADDED FUEL LEVEL AND NOTIFICATION FIELDS
# =============================================================================
...
class Ambulance(Base):
    ...
    fuel_consumption_rate = Column(Float, default=0.1)  # Fuel consumption per km
...
# =============================================================================
# DATABASE SERVICE - ADDED NEW METHODS
# =============================================================================
class Database:
    def __init__(self):
    ...
    # NEW METHOD: Find nearest ambulance to a hospital
    ...
    # NEW METHOD: Calculate distance between two coordinates
    ...
    # NEW METHOD: Update ambulance fuel level
...
# =============================================================================
# SERVICES - ENHANCED WITH NOTIFICATION AND TRACKING
# =============================================================================
...
class NotificationService:
    ...
    # NEW METHOD: Send pickup notification to driver
    ...
    # NEW METHOD: Send enroute notification to receiving hospital
...
class ReferralService:
    ...
    # NEW METHOD: Automatically assign nearest ambulance and send notifications
    ...
    # NEW METHOD: Mark patient as picked up and send enroute notification
    def mark_patient_picked_up(self, patient_id):
        ...
        # Send enroute notification to receiving hospital
...
class AmbulanceService:
    ...
    def get_available_ambulances_df(self):
        ...
        for ambulance in ambulances:
            data.append({
                ...
                'Fuel Level': f"{ambulance.fuel_level:.1f}%"  # Added fuel level
            })
    ...
    # NEW METHOD: Get ambulance with fuel information
...
class LocationSimulator:
    ...
    def start_simulation(self, ambulance_id, patient_id, start_lat, start_lng, end_lat, end_lng):
        ...
        # Get initial distance for fuel calculation
...
# =============================================================================
# UTILITIES - ENHANCED WITH GOOGLE MAPS AND FUEL TRACKING
# =============================================================================
class MapUtils:
    @staticmethod
    ...
    # NEW METHOD: Create real-time tracking map with Google Maps
    @staticmethod
    def create_real_time_tracking_map(patient, ambulance, hospitals_df):
        """Create a real-time tracking map showing ambulance movement"""
        if not ambulance or not patient:
        ...
        # Display ambulance information with fuel level
        ...
        if Config.GOOGLE_MAPS_API_KEY and ambulance.latitude and ambulance.longitude:
            ...
            # Create map with ambulance and hospitals
            map_html = f"""
            <iframe
                width="100%"
                height="500"
...
# =============================================================================
# DATA MODELS - UPDATED WITH FUEL LEVELS
# =============================================================================
hospitals_data = {
    'facility_name': [
    ...
    if existing_ambulances == 0:
        for amb_data in ambulances_data['ambulance_id']:
            ...
            ambulance = Ambulance(
                ...
                fuel_level=ambulances_data['fuel_level'][idx]  # Added fuel level
            )
...
class ReferralUI:
    ...
    def create_referral_form(self):
        ...
        with st.form("referral_form", clear_on_submit=True):
            ...
            # NEW: Ambulance assignment with fuel information
            ...
            if submitted:
                if not all([name, age, condition, referring_physician, referring_hospital, receiving_hospital]):
                    ...
                    if patient:
                        ...
                        # NEW: Auto-assign nearest ambulance if selected
    ...
    def display_active_referrals(self):
        ...
        if active_patients:
            ...
            for patient in active_patients:
                # Get ambulance fuel information if assigned
                ambulance_info = ""
    ...
    def display_patient_actions(self, patient):
        col1, col2, col3, col4 = st.columns(4)  # Added column for pickup action
        ...
        if active_transfers:
            for patient in active_transfers:
                with st.expander(f"🚑 {patient.name} - {patient.condition}", expanded=True):
                    ...
                    # NEW: Use enhanced real-time tracking map
                    ...
                    with col_not1:
                        pickup_status = "✅ Sent" if patient.pickup_notification_sent else "⏳ Pending"
                        st.metric("Pickup Notification to Driver", pickup_status)
                    with col_not2:
                        enroute_status = "✅ Sent" if patient.enroute_notification_sent else "⏳ Pending"
                        st.metric("Enroute Notification to Hospital", enroute_status)
        
        else:
            st.info("No active patient transfers to track")
            
        # Display all ambulances with fuel information
...
# =============================================================================
# MISSING UI CLASSES - ADDED HANDOVERUI, COMMUNICATIONUI, REPORTSUI, DRIVERUI
# =============================================================================
class HandoverUI:
    def __init__(self, db):
        self.db = db
...
# =============================================================================
# MAIN APPLICATION - UPDATED WITH ENHANCED SERVICES
# =============================================================================
class HospitalReferralApp:
    def __init__(self):
        ...
        self.notifications = NotificationService(self.db)  # Updated with db parameter
